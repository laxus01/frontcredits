...
export default {
  ...
  methods: {
    ...
    calculateDelay() {
      let limitDate = moment(this.credit.date).add(this.credit.mode, "d").format("YYYY-MM-DD")
      let currentDate = moment().format("YYYY-MM-DD")
      this.delayDays = currentDate > limitDate ? moment(currentDate).diff(limitDate, "d") : 0
    },
  },
  created() {
    this.getPayments()
